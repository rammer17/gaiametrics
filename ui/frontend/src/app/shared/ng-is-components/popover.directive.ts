...
export class PopoverDirective {
  ...
  private initPopover(): void {
    ...
    this.resize$!.pipe(takeUntil(this.destroy$)).subscribe(() => {
      if (this._popoverComponentRef)
        this._popoverComponentRef.instance.positionPopover();
    });
  }
  ...
  private projectComponentVariables(
...
class PopoverComponent {
  ...
  @HostBinding('@defaultAnimation') get getToggleDrawer(): any {
    return {
      ...
      params: {
        td: this.translateDirection,
        tv: this.translateValue,
        ad: this.animationDuration,
      },
    };
  }
